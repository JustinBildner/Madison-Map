...
public class FrontendMadisonMap implements IFrontendMadisonMap {
  ...
  public void DisplayShortestPath(String start, String end) {
    FDBackendMadisonMap map = new FDBackendMadisonMap();
    FDShortestPath shortestPath = map.getShortestPath(start, end);
    String pathNames = "";
  ...
  public void DisplayMinimumTree(String[] vertices) {
    FDBackendMadisonMap map = new FDBackendMadisonMap();
    FDShortestPath shortestPath = map.getMinimumSpanningTree(vertices);
    String pathNames = "";
  ...
  public static void main(String[] args) {
    ...
    System.out.println("(2) Display a minimum spanning tree between multiple hotspots");
    ...
    if (s1.equals("1")) {
      ...
      spots[0] = in.nextLine();
      System.out.println(
          "Enter up to 18 additional locations, or enter 'Q' to finish entering locations");
      for (int i = 1; i < 18; i++) {
        System.out.println("Location " + i + ":");
        if (in.nextLine().equals("Q") || in.nextLine().equals("q"))
          break;
        spots[i] = in.nextLine();
      }
      map.DisplayMinimumTree(spots); // Calls the method with the specified hotspots
    } else {
